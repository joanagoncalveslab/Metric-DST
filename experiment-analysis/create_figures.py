...
for (cancer, conf, pseudolabel) in [('BRCA', 90, 20), ('OV', 85, 20), ('CESC', 90, 10), ('SKCM', 90, 10), ('LUAD', 90, 10)]:
    for n in ['supervised', 'true_semi_supervised', 'balanced_semi_supervised', 'diversity']:
        df = pd.read_csv(f"W:/staff-umbrella/JGMasters/2122-mathijs-de-wolf/output/experiments-pipeline-what-settings/experiment-divergence-pipeline-{cancer}-knn-10-conf-{conf}-pseudolabels-{pseudolabel}/{n}_test_performance.csv", index_col=0).reset_index()
        df = df[["test_loss", "accuracy", "f1_score", "average_precision", "auroc"]]
