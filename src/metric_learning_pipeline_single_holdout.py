...
#     dataPath = holdoutexperimentFolder + f"train_seq_128_{args.cancer}.csv"
#     unlabeledDatapath = webDriveFolder + f"unknown_repair_cancer_{args.cancer}_seq_128.csv"
#     testDatapath = holdoutexperimentFolder + f"test_seq_128_{args.cancer}.csv" 

#     if args.output_file:
#         outputFolder = outputFolder + args.output_file + "/"
#     else:
#         outputFile = "experiment-single-holdout-"
#         i = 0
...
def main(dataset, test_dataset, unlabeled_dataset, args):
    ...
    for fold in range(total_number_of_folds):
        ...
        ml.train(fold, retrain=args.retrain, add_samples_to_convergence=args.early_stop_pseudolabeling, confidence=True)
        if args.single_fold:
            break
    gc.collect()
