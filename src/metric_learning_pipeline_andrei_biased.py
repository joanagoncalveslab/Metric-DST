...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument('-d', '--dataset', choices=['three_unbalanced', 'overlapped', 'rotated_moons', 'multidimension'], default='rotated_moons')
    parser.add_argument('-dd', '--dataset_dimension', type=int, default=42)
    parser.add_argument('-ss', '--sample_size', type=int, default=42)
    ...
    parser.add_argument("--bias", choices=['class_imbalance', 'reduce_samples', 'bias_2features', 'bias_most_important_feature', 'bias_multi_features', 'bias_balanced_multi_features'], default='bias_balanced_multi_features')
...
if __name__ == '__main__':
    ...
    if args.bias == 'class_imbalance':
        bias = {'name': None}
    if args.bias == 'reduce_samples':
        bias = {'name': None}
    if args.bias == 'bias_2features':
        bias = {'name': None}
    if args.bias == 'bias_most_important_feature':
        bias = {'name': None, 'a': 3, 'b': 4}
    if args.bias == 'bias_multi_features':
        bias = {'name': None, 'a': 3, 'b': 4}
    if args.bias == 'bias_balanced_multi_features':
        bias = {'name': None, 'a': 3, 'b': 4}
    ...
    with open(outputFolder / 'settings.txt', 'a+') as f:
        f.write('\n'.join([
            ...
            f'bias: {bias}',
            f'args: {args}',
            'Note: This is biased by code of Andrei. Test set is not balanced.'
            ''
        ]))
